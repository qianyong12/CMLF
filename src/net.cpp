...
void Net::init()
{
    ...
    for (int i = 0; i < layer_size; i++)
    {
        ...
        if (layer_mode[i] == MLP)
            layer[i].initWeight();
        getTime();
        std::cout << "layer " << i << " initial done\tlayer mode: " << layer_mode[i] << std::endl;
    }
...
void Net::update()
{
    for (int i = layer_size - 1; i >= 0; i--)
    {
        ...
        {
        case MLP:
            // std::cout << "mlp layer updating" << std::endl;
            for (int j = 0; j < layer[i].output_node; j++)
            {
                for (int k = 0; k < layer[i].input_node; k++)
                {
                    layer[i].weight.data[k][j] += -learning_rate * layer[i].loss.data[0][j] * layer[i].data_in.data[0][k];
                }
                layer[i].bias.data[0][j] += -learning_rate * layer[i].loss.data[0][j];
            }
            break;
        case RELU:
...
//                 for (int k = 0; k < layer[i].input_node; k++)
//                 {
//                     layer[i].weight_diff.data[k][j] += layer[i].loss.data[0][j] * layer[i].data_in.data[0][k];
//                 }
//                 layer[i].bias_diff.data[0][j] += layer[i].loss.data[0][j];
//             }
//             break;
//         case RELU:
//             for (int j = 0; j < layer[i - 1].output_node; j++)
